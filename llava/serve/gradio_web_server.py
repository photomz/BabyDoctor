...
def regenerate(state, request: gr.Request):
    # logger.warn('mode ' + image_process_mode)
...
instruction = "The following image is a radiology scan. Deeply analyze and diagnose this image."

def build_demo(embed_mode):
    textbox = gr.Textbox(show_label=False, placeholder="Enter text and press ENTER", visible=False, container=False, value=instruction)
    with gr.Blocks(title="BabyDoctor", theme=gr.themes.Soft()) as demo:
        state = gr.State()
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                expand_paths = lambda arr: [[f"{cur_dir}/examples/ROCO_{str(num).zfill(5)}.jpg"] for num in arr]
                gr.Examples(examples=expand_paths([27, 278,1259, 1747, 3257]), inputs=[imagebox])
                # gr.Examples(examples=[
                #     [f"{cur_dir}/examples/ROCO_00027.jpg"],
                #     [f"{cur_dir}/examples/ROCO_00278.jpg"],
                #     [f"{cur_dir}/examples/ROCO_00845.jpg"],
            ...
            with gr.Column(scale=6):
                chatbot = gr.Chatbot(elem_id="chatbot", label="Chat", visible=False, height=550)
                with gr.Row():
